...
void loop() {
  ...
  completionPercentage = (numberofTurns / steps) * 100;

  if (completionPercentage <= 80) {
    // Normal operation until 90% completion
    myservo.write(val); // Maintain the speed until 90% completion
    isSlowingDown = false; // Reset the slowdown flag
  } else {
    // Gradually slow down after 90% completion
    if (!isSlowingDown) {
      isSlowingDown = true; // Set the slowdown flag once
    }
    //if (val > 0 && numberofTurns <= steps) {
    if (numberofTurns <= steps) {
      val = 65; // Reduce the speed
      myservo.write(val);
    }else{
      myservo.write(0);
      val = 70; 
  ...
  ReadRawAngle(); // Get the sensor value
  correctAngle(); // Correct the angle
  checkQuadrant(); // Check quadrant, rotations, absolute angle

  // Serial printing
  Serial.println();
  Serial.println(totalAngle);

  // Handle input for steps if needed
  if (numberofTurns > steps) {
    myservo.write(0);
    Serial.print("Enter Steps: ");
    while (Serial.available() == 0) {
